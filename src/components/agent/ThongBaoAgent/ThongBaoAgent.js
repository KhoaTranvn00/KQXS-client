...
const ThongBaoAgent = () => {
	...
	const statusOptions = [
		{ label: "Hiển thị toàn bộ", value: "2" },
		{ label: "Chỉ hiện thị vé tồn", value: "1" },
	];

	useEffect(() => {
		(async () => {
			try {
				const response = await agentApi.getThongBao();
				if (response.success) {
					setVeDaDangs(response.thongbao[0].vesos);
	...
	useEffect(() => {
		(async () => {
			try {
				...
				const response = await agentApi.getThongBao(query);
				if (response.success) {
					setVeDaDangs(response.thongbao[0].vesos);
					setPagination(response.pagination);
				} else {
	...
	return (
		<div className="lottery-bought">
			<h1 ref={myRef}>
				Danh sách vé còn tồn
				{pagination ? ` (${pagination.totalVeE})` : ""}
			</h1>
			<h3>
				Tổng vé đã đăng
				...
				<>
					<table>
						<tr>
							...
							<th onClick={() => handleSortClick("soluong")}>Vé tồn</th>
							<th>Đài</th>
						...
						{veDaDangs.map((veDaMua, index) => (
							<tr>
								...
								<td>{veDaMua.vesoId.soluong}</td>
								<td>{veDaMua.vesoId.sold}</td>
								<td>{veDaMua.soVeE}</td>
